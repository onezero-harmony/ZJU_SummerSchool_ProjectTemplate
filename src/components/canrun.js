...
//  var myChart = echarts.init(document.getElementById('mainn'));
  ...
  for(var i = 0; i < state.data.length; i++){
    ...
    //if(tmp == NaN) console.log(state.data[i]['Funding Amount (USD)'] + "md");
    //else if (state.data[i]['Region'] == 'United States') console.log([countrydata[state.data[i]['Region']],state.data[i]['Funding Amount (USD)'],tmp]);
...
//  console.log(countrydata);
  ...
  try{
    console.log(state.data[0].hasOwnProperty('no'));
  }
  catch(e) {
    console.log(e);
  }
        ...
        if (main !== "") {
          ...
          mychart.on('click', function(params) {
            if (params.seriesType === 'map') { // 判断是否为地图区域被点击
                var country = params.name; // 获取省份名称
